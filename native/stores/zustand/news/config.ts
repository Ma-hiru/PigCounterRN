...
export const NewsConfig: ZustandConfig<initial & actions, initial> = (set, get, api) => ({
  ...initialState,
  ...
  addNews: (news: News) => {
    set((draft) => {
      draft.NewsList.push(news);
    });
  },
  deleteNews: (news: News) => {
    set(draft => {
      for (const [index, value] of draft.NewsList.entries()) {
        if (value.id === news.id) {
          draft.NewsList.splice(index, 1);
          return;
        }
      }
    });
...
interface actions {
  setNewsList: (newsList: NewsList) => void;
